...
#             # comport_list.append(port[0].device)
#     return comport_list
...
def find_teensys(sender, app_data, user_data, callback):
    ...
    for element in teensy_list:
        print(element)
        dpg.add_button(label=f"Connect to {element}", before="search_teensy_button", parent="search_teensy_button", tag=element, callback=connect_usb, user_data=False)
