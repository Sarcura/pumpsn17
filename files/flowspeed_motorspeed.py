...
def calculate_sorting_parameters(channel_m_per_s_total = 1.5, sample_to_sheath_ratio = 0.1, channel_area_mm2 = 0.003,
    ...
    print(f"Total volume to run through machine: [ml]: {cell_volume_ml:.2f}")
    print(f"Total estimated time [h]: {total_sorting_time:.2f}")
    print(f"Channel volume: [µl/s]: {channel_µl_per_s:.2f}")
    ...
    if calculate_medium_for_sorting_speed is True:
        ...
        if cell_volume_additional_ml > 0:
            ...
            print(f"Cell volume additionaly needed to sort with {sorting_speed} Hz: {additional_cell_media:.1f} ml")
            print(f"Addtionally {additional_sheath_fluid:.2f} ml of sheath will be needed.")
        ...
        # cell_volume_additional_ml = 0
            ...
            additional_cell_media = cell_volume_additional_ml*sample_to_sheath_ratio
            additional_sheath_fluid = cell_volume_additional_ml*(1-sample_to_sheath_ratio)
            print(f"No additional dilution needed. {additional_cell_media*-1:.2f} ml of medium could have been saved.")
            print(f"Addtionally {additional_sheath_fluid*-1:.2f} ml of sheath could have been saved.")
