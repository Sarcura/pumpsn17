...
void loop()  
{
  ...
  if(myTransfer.available())
  {
    ...
    digitalWrite (motor0_en, ParsingStruct.motor0_enable); //motor0
    digitalWrite (motor1_en, ParsingStruct.motor1_enable); //motor1
    digitalWrite (motor2_en, ParsingStruct.motor2_enable); //motor2
    digitalWrite (motor3_en, ParsingStruct.motor3_enable); //motor3
  ...
  motor3.moveTo(ParsingStruct.motor3_position);

  motor0.setSpeed(ParsingStruct.motor0_speed);
  motor1.setSpeed(ParsingStruct.motor1_speed);
  motor2.setSpeed(ParsingStruct.motor2_speed);
  motor3.setSpeed(ParsingStruct.motor3_speed);
  ...
  if (motor1.distanceToGo()== 0){
    digitalWrite (motor1_en, HIGH); //motor1
    ParsingStruct.motor1_enable = 1;
  }
  if (motor2.distanceToGo()== 0){
    digitalWrite (motor2_en, HIGH); //motor2
    ParsingStruct.motor2_enable = 1;
  }
