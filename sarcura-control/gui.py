...
from devices import Solenoids, Steppermotors
...
class serial_ui():
    ...
    def __init__(self):
        self.arduino_steppermotors = Steppermotors(findusbport_hwid="16C0:0483")
        self.arduino_solenoids = Solenoids(findusbport_hwid="16C0:0483")
        # self.arduino_steppermotors.hwid =  # should be changed by dropdown to search teensy, arduino..
        self.portList = []
        ...
        while dpg.is_dearpygui_running():
            if self.arduino_steppermotors.link:
                ...
                # recv_msg = self.arduino_steppermotors.read_serial()
                # if recv_msg:
                #     max_length = 122
    ...
    def calculate_motors(self):
        ...
        self.arduino_steppermotors.data_dict['stepspeed_1'] = int(round(stepspeed1))*dpg.get_value(self.nr_of_sorters)
        self.arduino_steppermotors.data_dict['stepspeed_2'] = int(round(stepspeed2))*dpg.get_value(self.nr_of_sorters)
        self.arduino_steppermotors.data_dict['stepspeed_3'] = int(round(stepspeed3))*dpg.get_value(self.nr_of_sorters)
        self.arduino_steppermotors.data_dict['stepspeed_4'] = int(round(stepspeed4))*dpg.get_value(self.nr_of_sorters)
    ...
    def update_ports_callback(self):
        hwid = dpg.get_value("hwid_text_search")
        if dpg.get_value("search_teensy"):
            self.arduino_steppermotors.hwid = "16C0:0483"
            self.arduino_solenoids.hwid = "16C0:0483"
        elif hwid:
            self.arduino_steppermotors.hwid = hwid
            self.arduino_solenoids.hwid = hwid
        else:
            self.arduino_steppermotors.hwid = None
            self.arduino_solenoids.hwid = None
        self.arduino_steppermotors.get_availabile_port_list()
        self.arduino_solenoids.get_availabile_port_list()
        # logging.info(f"Hwid: {self.arduino_steppermotors.hwid}")
        # logging.info(f"Hwid: {self.arduino_solenoids.hwid}")
        # logging.info(self.arduino_steppermotors.comport_list)
        dpg.configure_item("__listPortsTag_steppermotors", items=self.arduino_steppermotors.comport_list)
        dpg.configure_item("__listPortsTag_solenoids", items=self.arduino_solenoids.comport_list)
    ...
    def create_connections(self):
        with dpg.group(horizontal=True):
            dpg.add_button(tag="avPortsBtn_steppermotors", height=50, label="Refresh Available Ports", callback=self.update_ports_callback)
            with dpg.group():
                dpg.add_checkbox(label="Only search for Teensy microcontroller", tag="search_teensy", default_value=True)
                dpg.add_input_text(label="Optional hwid search", hint="16C0:0483 for teensy", width=150, tag="hwid_text_search")    
        dpg.add_separator()

        dpg.add_text(default_value=f"Steppermotor connection:")
        with dpg.group(horizontal=True):
            # dpg.add_combo(label=["No Ports available"], tag="__listPortsTag_steppermotors",
            #         width=300, callback=self.selected_port_callback_steppermotors)
            # set label to self.portList !!!!!!!
            # dpg.set_value(item="__listPortsTag_steppermotors", label=self.portList)

            if not self.portList:
                dpg.add_combo(["No Ports available"], tag="__listPortsTag_steppermotors",
                        width=300, callback=self.selected_port_callback_steppermotors)
            else:
                dpg.add_combo(self.portList, tag="__listPortsTag_steppermotors",
                    width=300, callback=self.selected_port_callback_steppermotors)
            dpg.add_loading_indicator(circle_count=2, tag="connected_indicator_steppermotors", speed=0)
        dpg.add_separator()

        dpg.add_text(default_value=f"Solenoid connection:")
        with dpg.group(horizontal=True):
            # if not self.portList_solenoids:
            if not self.portList:
                dpg.add_combo(["No Ports available"], tag="__listPortsTag_solenoids",
                        width=300, callback=self.selected_port_callback_solenoids)
            else:
                dpg.add_combo(self.portList, tag="__listPortsTag_solenoids",
                    width=300, callback=self.selected_port_callback_solenoids)
            dpg.add_loading_indicator(circle_count=2, tag="connected_indicator_solenoids", speed=0)
        dpg.add_separator()
    ...
    def create_send_solenoid_values(self, refresh=False):
        with dpg.group():
            # self.solenoids = int(dpg.get_value(item="nr_of_solenoids"))
            self.arduino_solenoids.nr_of_solenoids = 8
            # logging.info(self.solenoids)
            # i = 0
            # if refresh is True:
            #     while i <= self.solenoids:
            #         try:
            #             dpg.delete_item(label=f"solenoid_{i}")
            #         except:
            #             logging.info("all items deleted")
            #         i = i+1
            # else:
            #     pass
                # dpg.configure_item(item="refresh_solenoids", show=True)
            
            # self.solenoids = 8
            j = 0

            while j <= self.arduino_solenoids.nr_of_solenoids:
                dpg.add_checkbox(tag=f"solenoid_{j}", label=f"Solenoid {j}")
                j = j+1

            dpg.add_button(tag="sendSolBtn", label="Set solenoids",
                callback=self.send_solenoids_to_arduino, height=50)
        # for i in range(25):
        #     with dpg.table_row():
    ...
    def send_solenoids_to_arduino(self):
        # self.arduino_solenoids{}
        j = 0
        self.arduino_solenoids.data_list = []
        # self.arduino_solenoids.data_dict = []
        while j <= self.solenoids:
            # get value from solenoid checkbox j and convert bool to int
            sol_value = int(dpg.get_value(item=f"solenoid_{j}"))
            self.arduino_solenoids.data_list.append(sol_value)
            j = j+1
        # logging.info(self.arduino_solenoids.data_list)
        self.arduino_solenoids.send_to_arduino(self.arduino_solenoids.data_list)


    def create_send_pump_values(self):
        with dpg.group(horizontal=True):
    ...
    def create_automate_pumps(self):
        with dpg.group():
            dpg.add_checkbox(label="Automate Pump 1", tag="automate_pump_1")
            dpg.add_checkbox(label="Automate Pump 2", tag="automate_pump_2")
            dpg.add_checkbox(label="Automate Pump 3", tag="automate_pump_3")
            dpg.add_checkbox(label="Automate Pump 4", tag="automate_pump_4")
            dpg.add_input_int(label="Set interval time [s]", tag="interval_pumps",default_value=1, width=180)
            with dpg.group(horizontal=True):
                dpg.add_button(label="Start Automation", height=50, callback=self.automate_pumps, user_data=True, tag="start_auto_pumps")
                dpg.add_button(label="Stop Automation", height=50, callback=self.automate_pumps, user_data=False, tag="stop_auto_pumps")
                dpg.add_loading_indicator(tag="automated_indicator_pumps", speed=0)

    def create_automate_solenoids(self):
        with dpg.group():
            # dpg.add_checkbox(label="Automate Solenoid 1", tag="automate_solenoid_1")
            # dpg.add_checkbox(label="Automate Solenoid 2", tag="automate_solenoid_2")
            # dpg.add_checkbox(label="Automate Solenoid 3", tag="automate_solenoid_3")
            # dpg.add_checkbox(label="Automate Solenoid 4", tag="automate_solenoid_4")
            dpg.add_input_int(label="Set interval time [s]", tag="interval_solenoids",default_value=1, width=180)
            with dpg.group(horizontal=True):
                dpg.add_button(label="Start Automation", height=50, callback=self.automate_solenoids, user_data=True, tag="start_auto_solenoids")
                dpg.add_button(label="Stop Automation", height=50, callback=self.automate_solenoids, user_data=False, tag="stop_auto_solenoids")
                dpg.add_loading_indicator(tag="automated_indicator_solenoids", speed=0)
...
# dictionary_name[key] = value
        ...
        with dpg.window(tag="Primary Window", autosize=True) as self.prime_window:
            with dpg.tab_bar():
                ...
                with dpg.tab(label="Pumps"):
                    self.create_send_pump_values()

                with dpg.tab(label="Solenoids"):
                    # dpg.add_input_int(tag="nr_of_solenoids", default_value=1, max_value=100)
                    # dpg.add_button(label="Refresh solenoids", tag="refresh_solenoids", show=False, callback=self.create_send_solenoid_values(refresh=True))
                    self.create_send_solenoid_values()

                with dpg.tab(label="Automate Pumps"):
                    self.create_automate_pumps()

                with dpg.tab(label="Automate Solenoids"):
                    self.create_automate_solenoids()
    ...
    def dpg_setup(self):
        ...
        windowWidth  = 1100
        windowHeight = 435
        dpg.create_viewport(title='Sarcura Syringe Pump Control', width=windowWidth, height=windowHeight)
        dpg.setup_dearpygui()

    def selected_port_callback_steppermotors(self, Sender):
        ...
        self.arduino_steppermotors.port = self.SELECTED_DEVICE
        self.arduino_steppermotors.connect()

        if self.arduino_steppermotors.link:
            dpg.configure_item(item="connected_indicator_steppermotors", speed=1)
        else:
            dpg.configure_item(item="connected_indicator_steppermotors", speed=0)
        logging.info(f"User selected: {self.SELECTED_DEVICE}")

    def selected_port_callback_solenoids(self, Sender):
        self.SELECTED_DEVICE = dpg.get_value(Sender).split(' ')[0]
        logging.info("selected device: ")
        logging.info(self.SELECTED_DEVICE)
        # logging.info(self.SELECTED_DEVICE.count(" "))
        # logging.info(len(self.SELECTED_DEVICE))
        # logging.info(type(self.SELECTED_DEVICE))
        self.arduino_solenoids.port = self.SELECTED_DEVICE
        self.arduino_solenoids.connect()

        if self.arduino_solenoids.link:
            dpg.configure_item(item="connected_indicator_solenoids", speed=1)
        else:
            dpg.configure_item(item="connected_indicator_solenoids", speed=0)
...
################################### functions ################################
    ...
    def toggle_loop_pumps(self):
        global t_pumps
        self.pump_toggle = not self.pump_toggle
        self.arduino_steppermotors.data_dict_toggle = self.arduino_steppermotors.data_dict.copy()

        if dpg.get_value(item="automate_pump_1"):
            logging.info("pump1")
            self.arduino_steppermotors.data_dict_toggle['stepspeed_1'] = 0
        if dpg.get_value(item="automate_pump_2"):
            logging.info("pump2")
            self.arduino_steppermotors.data_dict_toggle['stepspeed_2'] = 0
        if dpg.get_value(item="automate_pump_3"):
            logging.info("pump3")
            self.arduino_steppermotors.data_dict_toggle['stepspeed_3'] = 0
        if dpg.get_value(item="automate_pump_4"):
            logging.info("pump4")
            self.arduino_steppermotors.data_dict_toggle['stepspeed_4'] = 0

        if self.pump_toggle is True:
            self.arduino_steppermotors.send_to_arduino(list(self.arduino_steppermotors.data_dict_toggle.values()))
        else:
            self.arduino_steppermotors.send_to_arduino(list(self.arduino_steppermotors.data_dict.values()))

        t_pumps = threading.Timer(dpg.get_value(item="interval_pumps"), self.toggle_loop_pumps)
        t_pumps.start()

    def toggle_loop_solenoids(self):
        global t_solenoids
        self.solenoid_toggle = not self.solenoid_toggle
        # copy the list into numpy, and invert it with -1
        self.arduino_solenoids.data_list_toggle = 1-np.array(self.arduino_solenoids.data_list).copy()
        # self.arduino_solenoids.data_list_toggle = 1-self.arduino_solenoids.data_list
        self.arduino_solenoids.data_list_toggle = self.arduino_solenoids.data_list_toggle.tolist()
        logging.info(self.arduino_solenoids.data_list)
        logging.info(self.arduino_solenoids.data_list_toggle)

        if self.solenoid_toggle is True:
            self.arduino_solenoids.send_to_arduino(self.arduino_solenoids.data_list_toggle)
        else:
            self.arduino_solenoids.send_to_arduino(self.arduino_solenoids.data_list)

        t_solenoids = threading.Timer(dpg.get_value(item="interval_solenoids"), self.toggle_loop_solenoids)
        t_solenoids.start()

    def automate_pumps(self, user_data):
        self.pump_toggle = True
        logging.info(user_data)
        if "start_auto_pumps" in user_data:
            logging.info("On")
            self.toggle_loop_pumps()
            dpg.configure_item(item="automated_indicator_pumps", speed=dpg.get_value(self.channel_m_per_s))
        else:
            logging.info("Off")
            t_pumps.cancel()
            dpg.configure_item(item="automated_indicator_pumps", speed=0)

    def automate_solenoids(self, user_data):
        self.solenoid_toggle = True
        logging.info(user_data)
        if "start_auto_solenoids" in user_data:
            logging.info("On")
            self.toggle_loop_solenoids()
            dpg.configure_item(item="automated_indicator_solenoids", speed=1)
        else:
            logging.info("Off")
            t_solenoids.cancel()
            dpg.configure_item(item="automated_indicator_solenoids", speed=0)
    ...
    def send_speed_to_arduino(self, sender, app_data, user_data):
        ...
        if user_data is None:
            ...
            if "speed" in user_data:
                ...
                self.arduino_steppermotors.data_dict['position_1'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict['position_2'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict['position_3'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict['position_4'] = self.arduino_steppermotors.max_speed
                self.calculate_motors()
            elif "go_to_endstops" in user_data and user_data["go_to_endstops"] == True:
                self.arduino_steppermotors.data_dict['position_1'] = 0
                self.arduino_steppermotors.data_dict['position_2'] = 0
                self.arduino_steppermotors.data_dict['position_3'] = 0
                self.arduino_steppermotors.data_dict['position_4'] = 0
                self.arduino_steppermotors.data_dict['stepspeed_1'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict['stepspeed_2'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict['stepspeed_3'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict['stepspeed_4'] = self.arduino_steppermotors.max_speed
            elif "set_speed_zero" in user_data and user_data["set_speed_zero"] == True:
                # disable on high, but not connected in hardware (?!)
                # self.arduino_steppermotors.data_dict['motor0_enable'] = 1
                self.arduino_steppermotors.data_dict['stepspeed_1'] = 0
                self.arduino_steppermotors.data_dict['stepspeed_2'] = 0
                self.arduino_steppermotors.data_dict['stepspeed_3'] = 0
                self.arduino_steppermotors.data_dict['stepspeed_4'] = 0
                dpg.configure_item(item="pumping_indicator", speed=0)
            elif "pump" and "fast_forward" in user_data:
                element = (user_data[1])
                logging.info(f"Moving pump {element} fast_forward")
                #only the according pump is set
                self.arduino_steppermotors.data_dict[f'position_{element}'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict[f'stepspeed_{element}'] = self.arduino_steppermotors.max_speed
            elif "pump" and "normal_forward" in user_data:
                element = (user_data[1])
                logging.info(f"Moving pump {element} fast_forward")
                #only the according pump is set
                self.arduino_steppermotors.data_dict[f'position_{element}'] = self.arduino_steppermotors.max_speed
                self.arduino_steppermotors.data_dict[f'stepspeed_{element}'] = int(self.arduino_steppermotors.max_speed/2)
            elif "pump" and "stop" in user_data:
                element = int((user_data[1]))
                logging.info(f"Stopping pump {element}")

                # logging.info(self.arduino_steppermotors.data_dict)
                # logging.info(type(element))

                self.arduino_steppermotors.data_dict[f'stepspeed_{element}'] = 0
                dpg.configure_item(item=f"setup_movement_indicator_{element}", speed=0)
            elif "pump" and "normal_backward" in user_data:
                element = (user_data[1])
                logging.info(f"Moving pump {element} fast_backward")
    ...
    #     elif not self.arduino_steppermotors.link:
    #         logging.info("Device is not connected")
    #     elif not msg_to_send:
    #         logging.info("No message.")
    #     else:
    #         self.arduino_steppermotors.write_to_serial(msg_to_send)
