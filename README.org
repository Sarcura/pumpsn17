...
*After modification, to create an executable use on of the following commands:

  #+BEGIN_SRC C :exports code      
  pyinstaller --python-option 3.9 .\pyserial_gui.spec --onefile --icon=sarcura.ico"
  pyinstaller --onefile --i sarcura.ico .\gui.py"
  pyinstaller --onefile --i sarcura.ico -w .\gui.py" # no console window
  #+END_SRC

or use pip installed auto-py-to-exe and create it via GUI
